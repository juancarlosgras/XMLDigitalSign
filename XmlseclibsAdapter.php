...
namespace XmlDigitalSignature\XmlDSig\Adapter;
...
class XmlseclibsAdapter implements AdapterInterface
{
    ...
    public function sign(DOMDocument $data, $serial)
    {
        ...
        foreach($data->getElementsByTagName("ExtensionContent") as $node){
        ...
        if($signNode == null){
            foreach($data->getElementsByTagName("UBLExtension") as $node){
                if(!$node->hasChildNodes()){
                    $signNode = $node;
                    break;
                }elseif($node->childNodes->length == 1){
            ...
            if($signNode == null) {
                $extensionElement = $data->createElement('ext:UBLExtension');
                $contentElement = $data->createElement('ext:ExtensionContent');
                $signNode = $data->getElementsByTagName("UBLExtensions")->item(0)->appendChild($extensionElement)->appendChild($contentElement);
            }
        }

        $objXMLSecDSig->sign($objKey, $signNode, $serial);
        ...
        $objXMLSecDSig->addObject($this->getPublicKey());
    }
